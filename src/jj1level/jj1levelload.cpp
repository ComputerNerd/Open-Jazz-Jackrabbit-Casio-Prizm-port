...
int JJ1Level::loadPanel () {
	...
	addobj(64*27,&panelAmmoramid[0]);
	sorted=(unsigned char *)objs[panelAmmoramid[0]].ptr;
	for (y = 0; y < 27; y++) {

		for (x = 0; x < 64; x++)
			sorted[(y * 64) + x] = rle_panel[(y * 64) + (x >> 2) + ((x & 3) << 4) + (55 * 320)];

	}
	initMiniSurface(&panelAmmo[0],sorted,64,27);
	//panelAmmo[0] = createSurface(sorted, 64, 27);

	addobj(64*27,&panelAmmoramid[1]);
	sorted=(unsigned char *)objs[panelAmmoramid[1]].ptr;
	for (y = 0; y < 27; y++) {

		for (x = 0; x < 64; x++)
			sorted[(y * 64) + x] = rle_panel[(y * 64) + (x >> 2) + ((x & 3) << 4) + (61 * 320)];

	}
	//panelAmmo[1] = createSurface(sorted, 64, 27);
	initMiniSurface(&panelAmmo[1],sorted,64,27);

	addobj(64*27,&panelAmmoramid[2]);
	sorted=(unsigned char *)objs[panelAmmoramid[2]].ptr;
	for (y = 0; y < 27; y++) {

		for (x = 0; x < 64; x++)
			sorted[(y * 64) + x] = rle_panel[(y * 64) + (x >> 2) + ((x & 3) << 4) + (68 * 320)];

	}

	//panelAmmo[2] = createSurface(sorted, 64, 27);
	initMiniSurface(&panelAmmo[2],sorted,64,27);
	
	addobj(64*27,&panelAmmoramid[3]);
	sorted=(unsigned char *)objs[panelAmmoramid[3]].ptr;
	for (y = 0; y < 27; y++) {

		for (x = 0; x < 64; x++)
			sorted[(y * 64) + x] = rle_panel[(y * 64) + (x >> 2) + ((x & 3) << 4) + (74 * 320)];

	}

	//panelAmmo[3] = createSurface(sorted, 64, 27);
	initMiniSurface(&panelAmmo[3],sorted,64,27);

	addobj(64*27,&panelAmmoramid[4]);
	sorted=(unsigned char *)objs[panelAmmoramid[4]].ptr;
	for (y = 0; y < 27; y++) {

		for (x = 0; x < 64; x++)
			sorted[(y * 64) + x] = rle_panel[(y * 64) + (x >> 2) + ((x & 3) << 4) + (86 * 320)];

	}
	initMiniSurface(&panelAmmo[4],sorted,64,27);
	//panelAmmo[4] = createSurface(sorted, 64, 27);
	//delete[] sorted;
	//delete[] pixels;
	//resizeobj(panelramid,SW*32);
